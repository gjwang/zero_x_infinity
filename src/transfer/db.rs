...
impl TransferDb {
    ...
    pub async fn create(&self, record: &TransferRecord) -> Result<i64, TransferError> {
        let transfer_id = sqlx::query_scalar::<_, i64>(
            ...
            INSERT INTO fsm_transfers_tb 
                (req_id, cid, user_id, asset_id, amount, transfer_type, source_type, state, created_at, updated_at)
            VALUES 
    ...
    fn row_to_record(&self, row: &sqlx::postgres::PgRow) -> Result<TransferRecord, TransferError> {
        ...
        Ok(TransferRecord {
            transfer_id: Some(row.get("transfer_id")),
            req_id,
            cid: row.get("cid"),
            source,
            target,
...
pub async fn record_operation(
    ...
    let insert_result = sqlx::query(
        r#"
        INSERT INTO transfer_operations_tb (req_id, op_type, service_type, result, error_message)
        VALUES ($1, $2, $3, $4, $5)
        ON CONFLICT (req_id, op_type, service_type) DO NOTHING
        "#,
    )
    .bind(req_id.to_string())
    .bind(op_type.as_str())
...
pub async fn check_operation(
    ...
    let row = sqlx::query(
        ...
        WHERE req_id = $1 AND op_type = $2 AND service_type = $3
        "#,
    )
    .bind(req_id.to_string())
    .bind(op_type.as_str())

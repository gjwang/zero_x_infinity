...
### QA报告
- 📄 **第一次交接**: [`dev-to-qa-handover.md`](./docs/agents/sessions/shared/dev-to-qa-handover.md)
- 📄 **QA拒绝报告**: [`qa-verification-rejected.md`](./docs/agents/sessions/shared/qa-verification-rejected.md)
  - Line 20-60: TC-P0-07失败根因分析
  - Line 62-76: TC-P0-04批准确认

### 设计文档
- 📘 [`docs/src/0x0B-a-transfer.md`](./docs/src/0x0B-a-transfer.md)
  - Section 1.5.7: Idempotency设计
  - Section 1.5.3: Amount validation

### 测试脚本
- 🧪 [`scripts/test_transfer_e2e.sh`](./scripts/test_transfer_e2e.sh)
...
## 🆕 UBSCore Audit Fixes (2025-12-26 04:40)

### 审计发现
QA独立验证发现 UBSCore 无运行时持久化：
- ❌ UBSCore WAL 不存在
- ❌ 崩溃后冻结余额丢失 
- ❌ 无 `ubscore_persistence` 配置选项

### 已修复 (3 commits)

| Commit | 修复内容 |
|--------|----------|
| `d578a60` | UBSC-GAP-01: WAL损坏时降级逻辑 |
| `3136f3c` | UBSCore 运行时 WAL v2 集成 |
| `1497055` | 跨服务同步协议 (ME↔UBSCore, Settlement↔ME) |

### 关键变更

**1. UBSC-GAP-01 - WAL 降级逻辑** (参考 SettlementRecovery)
```rust
// src/ubscore_wal/recovery.rs
// WAL损坏时不再返回Err，而是记录警告并继续使用快照
match reader.replay(...) {
    Ok(()) => { /* 正常 */ }
    Err(e) => {
        tracing::warn!("WAL replay failed, using snapshot only");
        next_seq_id = snapshot_seq + 1; // 回退到快照状态
    }
}
```

**2. UBSCore 运行时持久化** 
- `src/config.rs`: 新增 `UBSCorePersistenceConfig`
- `src/main.rs`: Gateway模式使用 `UBSCore::new_with_recovery()` 
- `config/dev.yaml`: 新增 `ubscore_persistence` 配置 (enabled: true)
...
# 2. 验证配置存在
grep -A4 "ubscore_persistence" config/dev.yaml
# 预期: enabled: true, data_dir: ./data/ubscore

# 3. 重新运行审计脚本
./scripts/audit_ubscore_adversarial.sh

# 预期结果:
# ✅ UBSCore WAL 存在 (./data/ubscore/wal/current.wal)
# ✅ 余额恢复成功
# ✅ ubscore_persistence 配置存在
```

---

*Handover Document v2.1*  
*Revision: Added UBSCore audit gap fixes (GAP-01, runtime persistence)*  

# 0x0D Settlement Service WAL & Snapshot 详细设计

> **Status**: 📋 DRAFT  
> **Author**: Architect Team  
> **Date**: 2024-12-25  
> **Parent**: [Service-Level Design](./0x0D-service-wal-snapshot-design.md)

---

## 1. 状态概述

| 状态 | 类型 | 说明 |
|------|------|------|
| **last_trade_id** | `u64` | 最后处理的成交 ID |
| **pending_settlements** | `Vec<Settlement>` | 待处理结算队列 |

---

## 2. 正常运行时流程

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                         Settlement Service 正常运行时流程                        │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│   Matching        Settlement Service              持久化           下游         │
│  ┌─────────┐     ┌─────────────────────────┐                                   │
│  │ Trade   │     │  1. 接收成交             │                                   │
│  │         │────▶│  2. 处理结算             │                                   │
│  └─────────┘     │  3. 写状态 WAL           │                                   │
│                  │  4. 持久化到 DB          │                                   │
│                  │  5. 通知下游             │                                   │
│                  └───────────┬─────────────┘                                   │
│                              │                                                  │
│                              ▼                                                  │
│                  ┌─────────────────────────┐                                   │
│                  │     状态 WAL            │                                   │
│                  │  ┌───────────────────┐  │                                   │
│                  │  │ type: Checkpoint  │  │                                   │
│                  │  │ last_trade_id: 100│  │                                   │
│                  │  │ timestamp         │  │                                   │
│                  │  │ type: Checkpoint  │  │                                   │
│                  │  │ last_trade_id: 200│  │                                   │
│                  │  └───────────────────┘  │                                   │
│                  └───────────┬─────────────┘                                   │
│                              │                                                  │
│                              ▼                                                  │
│                      TDengine / 其他存储                                         │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

---

## 3. 单笔成交结算流程

```rust
pub fn process_trade(&mut self, trade: Trade) -> Result<()> {
    // ============ Step 1: 验证成交 ID 连续性 ============
    if trade.trade_id != self.last_trade_id + 1 {
        return Err(format!(
            "Trade ID gap: expected {}, got {}",
            self.last_trade_id + 1,
            trade.trade_id
        ));
    }
    
    // ============ Step 2: 处理结算 ============
    // 这里执行实际的结算逻辑:
    // - 更新余额
    // - 计算手续费
    // - 生成 BalanceEvent
    self.settle_trade(&trade)?;
    
    // ============ Step 3: 持久化到 TDengine ============
    self.db.insert_trade(&trade)?;
    self.db.insert_balance_events(&trade.balance_events)?;
    
    // ============ Step 4: 写状态 WAL (轻量) ============
    // 只记录已处理到的进度
    self.last_trade_id = trade.trade_id;
    
    if self.last_trade_id % CHECKPOINT_INTERVAL == 0 {
        self.wal_writer.append(
            WalEntryType::SettlementCheckpoint,
            &CheckpointPayload {
                last_trade_id: self.last_trade_id,
                timestamp: now(),
            }
        )?;
    }
    
    // ============ Step 5: 通知下游 (可选) ============
    // 发送 WebSocket 更新等
    self.notify_downstream(&trade)?;
    
    Ok(())
}
```

### 关键点

| 步骤 | 说明 | 失败处理 |
|------|------|----------|
| 验证 trade_id | 确保无重复/遗漏 | 返回错误并重放 |
| 结算 | 更新余额、计算费用 | 事务回滚 |
| 持久化 | 写入 TDengine | 重试 |
| **写 Checkpoint** | **定期记录进度** | - |
| 通知下游 | WebSocket 等 | 可丢失 |

---

## 4. Snapshot 创建流程

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                         Settlement Snapshot 创建流程                            │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│   触发条件:                                                                     │
│   - 阈值: 每处理 10,000 笔成交                                                 │
│   - 关机: graceful shutdown 时                                                  │
│                                                                                 │
│   ┌──────────────────────────────────────────────────────────────────────┐     │
│   │  1. 暂停处理 (获取一致性点)                                           │     │
│   │     current_last_trade_id = self.last_trade_id                       │     │
│   │                                                                       │     │
│   │  2. 创建临时目录                                                      │     │
│   │     tmp_dir = snapshots/.tmp-{timestamp}/                            │     │
│   │                                                                       │     │
│   │  3. 写 metadata.json (轻量)                                           │     │
│   │     {                                                                 │     │
│   │       "format_version": 1,                                           │     │
│   │       "last_trade_id": 10000,                                        │     │
│   │       "created_at": "2024-12-25T20:00:00Z"                          │     │
│   │     }                                                                 │     │
│   │     // Settlement 的 Snapshot 非常轻量                                │     │
│   │     // 因为所有数据已持久化到 TDengine                                │     │
│   │     // Snapshot 只记录进度                                           │     │
│   │                                                                       │     │
│   │  4. 写 COMPLETE 标记                                                  │     │
│   │  5. 原子重命名                                                        │     │
│   │     rename(.tmp-xxx -> snapshot-{last_trade_id})                     │     │
│   │  6. 更新 latest 符号链接                                              │     │
│   │  7. 恢复处理                                                          │     │
│   └──────────────────────────────────────────────────────────────────────┘     │
│                                                                                 │
│   最终目录结构:                                                                 │
│   snapshots/                                                                    │
│   ├── snapshot-10000/                                                          │
│   │   ├── metadata.json                                                        │
│   │   └── COMPLETE                                                             │
│   └── latest -> snapshot-10000/                                                │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

**注意**: Settlement 的 Snapshot 非常轻量，因为：
- 所有 Trade 和 BalanceEvent 已持久化到 TDengine
- Snapshot 只需记录 `last_trade_id` (处理进度)
- 无需存储大量状态数据

---

## 5. 恢复流程 (启动时)

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                         Settlement Service 恢复流程 (启动时)                    │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│   ┌──────────────────────────────────────────────────────────────────────┐     │
│   │  1. 检查 snapshots/latest 是否存在                                    │     │
│   │                                                                       │     │
│   │  2. 如果存在:                                                         │     │
│   │     a. 检查 COMPLETE 标记                                             │     │
│   │     b. 读取 metadata.json                                             │     │
│   │        last_trade_id = metadata.last_trade_id  // 例如 10000          │     │
│   │                                                                       │     │
│   │  3. 如果不存在 (冷启动):                                              │     │
│   │     last_trade_id = 0                                                 │     │
│   │                                                                       │     │
│   │  4. 重放 WAL (从最后一个 Checkpoint)                                  │     │
│   │     // WAL 只有轻量的 Checkpoint 记录                                 │     │
│   │     // 主要用于验证 last_trade_id 的准确性                            │     │
│   │     for entry in wal.replay():                                        │     │
│   │         if entry.type == SettlementCheckpoint:                        │     │
│   │             last_trade_id = max(last_trade_id, entry.last_trade_id)  │     │
│   │                                                                       │     │
│   │  5. 请求 ME 重放 (从 last_trade_id + 1 开始)                          │     │
│   │     request = ReplayRequest { from_trade_id: last_trade_id + 1 }     │     │
│   │     matching.replay_trades(request, |trade| {                         │     │
│   │         self.process_trade(trade);                                    │     │
│   │     });                                                                │     │
│   │                                                                       │     │
│   │  6. 恢复完成                                                          │     │
│   │     log: "Recovered Settlement to trade_id={}", self.last_trade_id   │     │
│   │                                                                       │     │
│   │  7. 开始正常结算                                                      │     │
│   └──────────────────────────────────────────────────────────────────────┘     │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

---

## 6. 时序图

### 6.1 正常运行

```
      Matching      Settlement        State WAL       TDengine
          │             │                 │              │
  Trade   │             │                 │              │
  ────────▶             │                 │              │
          │  settle     │                 │              │
          │────────────▶│                 │              │
          │             │  insert         │              │
          │             │─────────────────────────────▶│
          │             │  OK             │              │
          │             │◀─────────────────────────────│
          │             │  checkpoint     │              │
          │             │────────────────▶│              │
          │             │  OK             │              │
          │             │◀────────────────│              │
          │             │                 │              │
```

### 6.2 恢复流程

```
      Startup    Snapshot      WAL        Matching    Settlement
          │         │           │             │            │
          │  read   │           │             │            │
          │────────▶│           │             │            │
          │ id=10000│           │             │            │
          │◀────────│           │             │            │
          │         │  replay   │             │            │
          │─────────────────────▶             │            │
          │         │  checkpoints            │            │
          │◀─────────────────────             │            │
          │         │           │  replay(10001+)          │
          │─────────────────────────────────▶│            │
          │         │           │  Trades     │            │
          │◀─────────────────────────────────│            │
          │         │           │             │  re-settle │
          │───────────────────────────────────────────────▶│
          │         │           │             │            │
       Ready       │           │             │            │
```

---

## 7. WAL Entry Types

| Type | Payload | 说明 |
|------|---------|------|
| `SettlementCheckpoint` | CheckpointPayload | 结算进度检查点 |

**CheckpointPayload**:
```rust
struct CheckpointPayload {
    last_trade_id: u64,      // 最后处理的 trade_id
    timestamp: u64,          // 时间戳
}
```

**Checkpoint 间隔**: 每 1,000 笔成交

---

## 8. Snapshot 格式

```rust
struct SettlementSnapshot {
    format_version: u32,
    last_trade_id: u64,      // 最后处理的 trade_id
    created_at: u64,
}
```

**特点**: 
- 非常轻量 (只有元数据)
- 所有业务数据已在 TDengine 中
- Snapshot 只记录处理进度

---

## 9. 配置参数

```rust
pub struct SettlementConfig {
    // WAL
    pub wal_dir: PathBuf,
    pub checkpoint_interval: u64,       // 1,000 trades
    
    // Snapshot
    pub snapshot_dir: PathBuf,
    pub snapshot_threshold: u64,        // 10,000 trades
    pub snapshot_keep_count: usize,     // 3
    
    // Matching 连接
    pub matching_replay_endpoint: String,
    
    // TDengine
    pub tdengine_url: String,
}
```

---

## 10. 幂等性保证

Settlement 必须确保幂等性，避免重复处理：

```rust
pub fn ensure_idempotency(&self, trade: &Trade) -> Result<()> {
    // 检查 TDengine 是否已存在该 trade_id
    if self.db.trade_exists(trade.trade_id)? {
        warn!("Trade {} already processed, skipping", trade.trade_id);
        return Ok(());
    }
    
    // 检查 trade_id 连续性
    if trade.trade_id != self.last_trade_id + 1 {
        return Err(format!(
            "Trade ID gap detected: expected {}, got {}",
            self.last_trade_id + 1,
            trade.trade_id
        ));
    }
    
    Ok(())
}
```

---

*Document created: 2024-12-25*

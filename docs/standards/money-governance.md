...
`u64` 最大值约为 `1.84 * 10^19`。

| 资产精度 | 最大可处理金额 | 示例 |
|----------|----------------|------|
| 8 位 (BTC) | 184,467,440,737 BTC | 远超总供应量，安全 |
| 18 位 (ETH) | **18.44 ETH** | ⚠️ 存在溢出风险 |
| 18 位 (SHIB) | 极小数量 | ⚠️ 无法处理大户交易 |

> [!WARNING]
> **当前决策**: 对于 18 位精度资产，系统在初期阶段可接受此限制。**未来升级路径**: 将内部表示从 `u64` 升级为 `u128`。

...
|------|--------|-----------------------------|
| **启动阶段 (Startup)** | 致命 | **优雅终止**。载入配置时遇到非法数据，打印清晰错误信息后以非零退出码终止（禁止 panic）。 |
...
#### 2.7.2 安全强化路线图 (Security Hardening Roadmap)

以下是金融安全专家建议的待实现功能，按优先级排列：

**🔴 高优先级 - 上线前必须解决**

| 功能 | 描述 | 当前状态 |
|------|------|----------|
| 提币限额检查 | 单笔/日/月限额，防止大额提币 | 未实现 |
| 热钱包/冷钱包分离 | 减少热钱包被盗时的损失 | 架构待设计 |
| 提币二次确认 | 大额提币需邮件/2FA 确认 | 未实现 |
| 充值延迟入账 | 多节点确认，防 RPC 劫持 | 部分实现 (confirmations) |

**🟡 中优先级 - 建议尽快实现**

| 功能 | 描述 |
|------|------|
| 独立审计日志 | 余额变动的不可篡改日志，独立于 WAL |
| 对账系统 | 定期核对：Hot wallet vs 用户余额总和 |
| API Rate Limiting | 防止恶意刷单/DDoS |
| IP 白名单 | API Key 绑定 IP |

**🟢 架构层面建议**

| 功能 | 描述 |
|------|------|
| 出金审批流程 | 大额 → 人工审批，时间锁延迟 |
| 熔断机制 | 短时间大量提币 → 自动暂停 |
| 零信任架构 | 不信任任何组件（Sentinel/ME 都需验证） |

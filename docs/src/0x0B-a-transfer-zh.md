...
| 检查项 | 攻击向量 | 验证逻辑 | 错误码 |
|---|---|---|---|
| **client_order_id 唯一** | 重复提交 | 如提供 `client_order_id`，检查是否已存在 | `DUPLICATE_REQUEST` (返回原结果) |
...
CREATE TABLE transfers_tb (
    ...
    req_id        VARCHAR(64) UNIQUE NOT NULL,  -- 客户端幂等键
...
> [!IMPORTANT]
> **`req_id` 由服务端生成**，不是客户端。
> 客户端如果需要幂等性，应使用 `client_order_id` 字段（可选），服务端会检查重复并返回已有结果。
...
| `DUPLICATE_REQUEST` | `client_order_id` 已处理。返回原始结果。 |
...
function handle_transfer_request(request, auth_context):
    ...
    if request.client_order_id:
        existing = db.find_by_client_order_id(request.client_order_id)
        if existing:
...
| **ATK-010** | 重复 client_order_id | 同一 client_order_id 发两次 | 第二次返回第一次结果 |

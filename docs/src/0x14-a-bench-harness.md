...
### 6. Missing Data in Golden Dataset

> [!WARNING]
> **Critical Gap**: The golden CSV files only contain order data, but **NOT the user accounts data** generated by `UserCurrencyAccountsGenerator.generateUsers()`.

**Impact**:
- UID matching requires knowing the exact `List<BitSet>` of user currency accounts
- This data is generated using `Random(1)` + `ParetoDistribution(JDKRandomGenerator(0), 1, 1.5)`
- Without this intermediate data, we cannot verify UID mapping exactly

**Solution Options**:
1. **Export user accounts** - Modify `RustPortingDataDumper.java` to also dump user accounts data
2. **Re-implement Pareto** - Implement Apache Commons Math's exact Pareto distribution in Rust
3. **Accept partial match** - Price/Size/Action verification is sufficient for core algorithm validation
...
> [!WARNING]
> **关键缺口**: 黄金 CSV 文件只包含订单数据，**不包含** `UserCurrencyAccountsGenerator.generateUsers()` 生成的用户账户数据。

**影响**:
- UID 匹配需要知道精确的 `List<BitSet>` 用户货币账户
- 此数据使用 `Random(1)` + `ParetoDistribution(JDKRandomGenerator(0), 1, 1.5)` 生成
- 缺少此中间数据，无法精确验证 UID 映射

**解决方案**:
1. **导出用户账户** - 修改 `RustPortingDataDumper.java` 同时导出用户账户数据
2. **重新实现 Pareto** - 在 Rust 中实现 Apache Commons Math 的精确 Pareto 分布
3. **接受部分匹配** - Price/Size/Action 验证足以验证核心算法正确性

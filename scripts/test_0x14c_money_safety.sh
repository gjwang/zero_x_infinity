#!/bin/bash
# =============================================================================
# Phase 0x14-c: Money Safety Complete Functional QA Test
# =============================================================================
# 参考: scripts/test_0x14b_qa.sh
# 
# Automates:
# 1. Database Initialization (PostgreSQL & TDengine)
# 2. Gateway Startup (Matching Engine in CI mode)
# 3. Execution of QA Test Suite (Agent A/B/C)
# 4. Cleanup
# =============================================================================
...
# Source Unified Utilities - 复用项目基础设施
...
# Define Log File
GATEWAY_LOG="${LOG_DIR}/gateway_0x14c_qa.log"
...
log_info "Running QA 0x14-c Money Safety test suite..."
cd "${PROJECT_ROOT}"
if command -v uv >/dev/null 2>&1; then
    uv run python3 "scripts/tests/0x14c_money_safety/run_all_tests.py"
else
...
if [ $TEST_EXIT_CODE -eq 0 ]; then
    log_success "QA 0x14-c Money Safety Verification PASSED! ✅"
else
    log_err "QA 0x14-c Money Safety Verification FAILED! ❌"
